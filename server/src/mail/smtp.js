...
/**
 * Email sent when a nonconformance is escalated due to lack of response.
 */
export async function sendEscalationEmail({
    ...
    if (!isSmtpConfigured()) {
        console.warn('[NC-ESCALATION] SMTP not configured; skipping escalation email.');
        return { sent: false, skipped: true };
    }

    const safeEscalationEmail = escapeHtml(escalationEmail);
    const safeAssignee = escapeHtml(assigneeName || 'The assignee');
    const safeAudit = escapeHtml(auditName || 'an audit');
    const safeTitle = escapeHtml(findingTitle || ncNumber || 'Nonconformance');
    const safeNc = escapeHtml(ncNumber || '');
    const base = getFrontendBaseUrl();
    const loginUrl = `${base}/login`;
    ...
    const subject = safeNc
        ? `Escalation: Nonconformance ${safeNc} overdue`
        : `Escalation: A nonconformance is overdue`;

    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1e293b;">
            <div style="background-color: #dc2626; padding: 24px; text-align: center; border-radius: 8px 8px 0 0;">
                <h1 style="color: white; margin: 0; font-size: 20px;">Nonconformance Escalated</h1>
            </div>
            <div style="padding: 24px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 8px 8px;">
                <p style="margin-top: 0;">Hello,</p>
                <p>
                    A nonconformance assigned to <strong>${safeAssignee}</strong> on the audit <strong>${safeAudit}</strong> has passed its escalation date without a response.
                </p>
                <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 16px; margin: 20px 0;">
                    ${safeNc ? `<p style="margin: 0 0 8px 0; font-size: 13px; color: #991b1b;">NC number</p>
                    <p style="margin: 0 0 12px 0; font-size: 15px; font-weight: bold; color: #7f1d1d;">${safeNc}</p>` : ''}
                    <p style="margin: 0 0 6px 0; font-size: 13px; color: #991b1b;">Finding</p>
                    <p style="margin: 0; font-size: 15px; font-weight: bold; color: #7f1d1d;">${safeTitle}</p>
                </div>
                <p style="margin: 24px 0;">
                    <a href="${ctaUrl}" style="display: inline-block; background: #dc2626; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: bold;">
                        View Nonconformance
                    </a>
                </p>
                <p style="font-size: 13px; color: #64748b;">
                    Or sign in at <a href="${loginUrl}" style="color: #dc2626;">${loginUrl}</a>
                </p>
                <p style="margin-top: 24px; font-size: 11px; color: #94a3b8;">This is an automated escalation message.</p>
            </div>
        </div>
    `;
    ...
    await transporter.sendMail({
        from: getSmtpFromAddress(),
        to: safeEscalationEmail,
        subject,
        html,
        text: `A nonconformance (${findingTitle || ncNumber || 'Finding'}) assigned to ${assigneeName || 'the assignee'} on ${auditName || 'an audit'} is overdue and has been escalated. View it here: ${ctaUrl}`,
    });
